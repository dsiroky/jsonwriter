...
env = Environment(toolpath=[os.path.join(PROJECT_ROOT, "tools/scons")])

env["OPTIMIZATION"] = GetOption("optimization")

for cmd in ("CC", "CXX", "LINK", "AR", "RANLIB"):
...
if "gcc" in env["CC"]:
    env.AppendUnique(CCFLAGS=["-fmax-errors=10", ])
if "clang" in env["CC"]:
    env.AppendUnique(CCFLAGS=["-ferror-limit=10", ])
