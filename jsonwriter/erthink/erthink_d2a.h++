...
inline void make_digits(PRINTER &printer, const uint64_t top, uint64_t delta,
  ...
  do {
    ...
    case 0:
      digit = body;
      if (erthink_unlikely(tail < delta)) {
      early_last:
  ...
  } while (erthink_unlikely(digit == 0));

  while (true) {
    if (erthink_unlikely(!printer.mantissa_digit(static_cast<char>(digit) + '0')))
      goto early_skip;
    switch (--kappa) {
    default:
      assert(false);
    ...
    if (erthink_unlikely(left < delta)) {
      if (erthink_likely(digit))
        goto early_last;
      ++kappa;
      goto early_skip;
  ...
  while (erthink_likely(printer.mantissa_digit(static_cast<char>(digit) + '0') &&
                tail > delta)) {
    --kappa;
#if ERTHINK_D2A_AVOID_MUL
    tail += tail << 2;   // *= 5
...
inline void convert(PRINTER &printer, diy_fp diy) cxx11_noexcept {
  if (erthink_unlikely(diy.e == 0x7ff - grisu::GRISU_EXPONENT_BIAS))
    return (diy.f - grisu::IEEE754_DOUBLE_IMPLICIT_LEAD) ? printer.nan()
                                                         : printer.inf();
  if (erthink_unlikely(diy.f == 0))
    return printer.zero();
  ...
  if (!printer.is_accurate() && erthink_unlikely(diy.e >= -52 && diy.e <= lead_zeros) &&
...
public:
  ...
  constexpr operator int() const noexcept {
    return erthink_likely(is_finite())
               ? (erthink_likely(is_normal()) ? FP_NORMAL
